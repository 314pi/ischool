				...
				QApplication::desktop()->screenNumber( this ) )
{
	...
	setWindowTitle( tr( "iSCHOOL" ) + " " + ISCHOOL_VERSION );
						...
						LocalSystem::Path::personalConfigDataPath() ) == false )
	{
		...
		QMessageBox::information( this, tr( "No write access" ),
			...
			"For running iSCHOOL, make sure you're permitted to "
...
/*	setCentralWidget( hbox );
	...
	ToolButton * overview_mode = new ToolButton(
			...
			tr( "This is the default mode in iSCHOOL and allows you "
...
/*	ToolButton * remotelogon = new ToolButton(
	...
	IschoolVncConnection * conn = new IschoolVncConnection( this );
	// attach IschoolCoreConnection to it so we can send extended iSCHOOL commands
	m_localICA = new IschoolCoreConnection( conn );

	conn->setHost( QHostAddress( QHostAddress::LocalHost ).toString() );
	conn->setPort( IschoolCore::config->coreServerPort() );
	...
	if( !conn->waitForConnected( 5000 ) )
	{
		QMessageBox::information( this,
			tr( "Could not contact iSCHOOL service" ),
			tr( "Could not contact the local iSCHOOL service. It is likely "
				"that you entered wrong credentials or key files are "
				"not set up properly. Try again or contact your "
				"administrator for solving this problem using the iSCHOOL "
				"Management Console." ) );
	...
	m_localICA->setRole( IschoolCore::role );
	m_localICA->startDemoServer( IschoolCore::config->coreServerPort(),
									IschoolCore::config->demoServerPort() );

//	##ISCHOOL2: m_localISD->hideTrayIcon();
	...
	m_systemTrayIcon.setToolTip( tr( "iSCHOOL Master Control" ) );
...
{
	if( IschoolCore::initAuthentication() )
	{
		return true;
	}

	if( IschoolCore::role != IschoolCore::RoleTeacher )
	{
	...
	QMessageBox::information( NULL,
			...
			tr(	"No authentication key files were found or your current ones "
				"are outdated. Please create new key files using the iSCHOOL "
				"Management Console. Alternatively set up logon authentication "
				"using the iSCHOOL Management Console. Otherwise you won't be "
				"able to access computers using iSCHOOL." ) );
...
void MainWindow::aboutISCHOOL( void )
{
